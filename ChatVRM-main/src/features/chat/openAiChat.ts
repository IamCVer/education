...
// 【C】对话模型：通义千问 API（非流式）
const DASHSCOPE_KEY = typeof window !== 'undefined' 
  ? process.env.NEXT_PUBLIC_DASHSCOPE_KEY 
  : '';

export async function getChatResponse(messages: Message[], apiKey: string) {
  // 使用通义千问 API
  if (!DASHSCOPE_KEY) {
    throw new Error("未配置 DASHSCOPE_KEY");
  }
  ...
  try {
    ...
    const message = data.choices?.[0]?.message?.content || "回答生成失败";
